...
export function createUI(root: HTMLElement, actions: UIActions): PresentationUI {
  root.innerHTML = `
    <div class="topbar">
      <div class="identity">
        <div class="eyebrow">${MT1_BUILD_INFO.candidateId} / PROPULSION HANDOVER</div>
        <div class="title">FIXED CORE · HOLLOW THRUST CAN · DEPLOYED CAPTURE</div>
        <div class="subtitle">UNLOCK → FRONT COMPACT → REAR PACKAGE → REORIENT → SEAT → DRIVE</div>
        <div class="provenance">FREEZE ${MT1_BUILD_INFO.freezeId} · ARCHIVE ${MT1_BUILD_INFO.candidateSha256.slice(0, 12)}… · INSPECTION ${MT1_BUILD_INFO.inspectionSurface}</div>
      </div>

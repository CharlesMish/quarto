...
export function createUI(root: HTMLElement, actions: UIActions): PresentationUI {
  root.innerHTML = `
    <div class="topbar">
      <div class="identity">
        ...
        <div class="title">OPEN-STERN CHINE HULL · BOOK POCKETS · EXPOSED HANDOVER</div>
    ...
    <div class="help">
      ...
      SPACE play machineT · R reset · D debug · S section · B body · 1–9 machine poses
    </div>
    <section class="control-deck">
      ...
      <div class="button-row">
        ...
        <div class="actions">
          ...
          <button data-cam="body">BODY 3/4</button>
          <button data-cam="driveBody">DRIVE 3/4</button>
          ...
          <button id="bodySectionBtn">BODY SECTION</button>
          <button id="bodySpreadBtn">BODY SPREAD</button>
          <button id="bodyMidBtn">BODY MID</button>
          <button id="bodyDriveBtn">BODY DRIVE</button>
        </div>
      </div>
      <div class="h1-sequence"><b>BODY REVIEW</b> BODY SPREAD → BODY MID → BODY DRIVE → SIDE / 3/4 / REAR → BODY SECTION → BODY OFF → PROP + PROP SECTION</div>
    </section>
  ...
  const bodySectionBtn = root.querySelector<HTMLButtonElement>("#bodySectionBtn")!;
  ...
  bodySectionBtn.addEventListener("click", () => {
    bodySectionBtn.classList.toggle("active", actions.toggleBodySection());
  });
  root.querySelector<HTMLButtonElement>("#bodySpreadBtn")!.addEventListener("click", () => {
    actions.setMachineT(0);
    actions.setCamera("body");
  });
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (event.key.toLowerCase() === "b") bodyConceptBtn.click();
